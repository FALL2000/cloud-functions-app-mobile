    ...
    try {
        ...
        switch (action) {
            ...
            case 'UPDATE-APPROVAL-STATUS':
                ...
                return data.userNote ? await updateStatusApproval(data.transfertId, data.approvalId, data.status, context, data.userNote) : await updateStatusApproval(data.transfertId, data.approvalId, data.status, context);
            default:
