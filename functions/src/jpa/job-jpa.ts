...
export class JpaJob {
    ...
    public async  createAsyncJobTriggerComplexe (recordIds:any[], univers:string){
        info("createNextAsyncJobTriggerSimple")
        info(recordIds)
        const __asyncJobTrigger:AsyncJob= AsyncJob.buildComplexeQueuedJob();
        __asyncJobTrigger.recordIds= recordIds;
        __asyncJobTrigger.univers= univers;
        return await this.create(__asyncJobTrigger.toSave())
    }
}
